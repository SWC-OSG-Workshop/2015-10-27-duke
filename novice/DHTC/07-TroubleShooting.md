...
-- Submitter: login.duke.ci-connect.net : <192.170.227.195:56174> : login.duke.ci-connect.net 
---
...
This bit has the full path of the file that Condor tried to transfer back to `login.duke.ci-connect.net`. The reason why the file transfer failed is because `outputfile` was never created on the worker node. Remember that at the beginning we said that the user specifically requested `transfer_outputfiles = outputfile`! Condor could not complete this request, and so the job went into Held state instead of finishing normally.
