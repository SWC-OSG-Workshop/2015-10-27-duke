...
$(BOOK_MD) : $(MOST_SRC) bin/make-book.py
	   ~/anaconda/bin/python bin/make-book.py $(MOST_SRC) > $@
...
check :
	~/anaconda/bin/python bin/swc_index_validator.py ./index.html
...
gloss : $(INDEX)
	~/anaconda/bin/python bin/gloss.py gloss.md $(patsubst %.md,$(SITE)/%.html,$(MOST_SRC))
